...
def sparse_continents
  ...
  FROM
    countries
  WHERE
  -- select the continents that are not disqualifieds
    continent NOT IN (
      -- returns continents for those cities, AKA continents who are not candidates
    SELECT
      continent
    FROM
      countries
    WHERE
      name IN (
        -- returns countries with a population greater than 25000000
        SELECT
          name
        FROM
          countries
